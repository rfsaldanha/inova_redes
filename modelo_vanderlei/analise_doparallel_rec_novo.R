...
library(readr)
processamento_log <- read_delim("processamento_log_enviados_mes.csv", 
...
# Criar arquivo de log
 log_file <- "processamento_log_enviados_mes.csv"
...
ajustar_e_plotar <- function(df, t, municipio) {
  ...
  df_mun <- df_mun %>%
    mutate(date = lubridate::floor_date(as.Date(DT_INTERN), unit = "month")) %>%
    group_by(date) %>%
  ...
  tryCatch({
    ...
    RPostgres::dbAppendTable(
      con,
      name = DBI::Id(schema = "inova", table = "resul_mod_enviados_mes"),
      value = plot_data
    )
    RPostgres::dbDisconnect(con)
    print("Insert")
...
# }    
    ...
    df <- tb_emergencias_tips %>%
      filter(MUNIC_MOV == munic, MUNIC_RES != MUNIC_MOV, TIPO == t) %>%
      mutate(municipio_geocodigo = MUNIC_MOV, ano_e = format(DT_INTERN, "%Y"),
...
teste<-function(x){
  ...
  library(readr)
  resul_mod_enviados_para_novo <- read_delim("resul_mod_enviados_para_novo.csv", 
                                             delim = ";", escape_double = FALSE, 
                                             col_names = FALSE, 
                                             trim_ws = TRUE)
  #head(resul_mod_enviados_para_novo)

  ####Verificar se codigo municipio está correto 
  teste <- lapply(substring(resul_mod_enviados_para_novo$X6,1,6), FUN = verif_cod)
  resul_mod_enviados_para_novo$teste <- unlist(teste)
  

  
  (table(is.na(resul_mod_enviados_para_novo$teste))/length(resul_mod_enviados_para_novo$X1)*100)
  
  names(resul_mod_enviados_para_novo) <- c("data","obs","pred","max","min","cod6","tipo", "teste")
  
  (table(resul_mod_enviados_para_novo$teste == resul_mod_enviados_para_novo$cod6)
    /length(resul_mod_enviados_para_novo$cod6)*100)
  
  resul_mod_enviados_para_novo_1 <- resul_mod_enviados_para_novo %>% 
    dplyr::filter(data >= '2008-01-01' & data <= '2024-12-31') %>%
    dplyr::filter(teste > 0) %>% 
    #dplyr::filter(is.na(pred) == T) %>% 
    dplyr::select(c("data","obs","pred","max","min","cod6","tipo"))
  
  table(resul_mod_enviados_para_novo_1$tipo)
  ...
  summary(resul_mod_enviados_para_novo_1)
  
  write.table(x =   resul_mod_enviados_para_novo_1, 
              file = "resul_mod_enviados_testado.csv", 
